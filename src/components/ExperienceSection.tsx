      ...
      <div className="section-padding">
        <div className="max-w-5xl mx-auto">
          <motion.div
            ...
            className="mb-14"
          >
            <p className={`text-primary font-heading text-sm tracking-[0.3em] uppercase mb-3 ${isRTL ? 'font-arabic' : ''}`}>{t("exp.label")}</p>
          ...
          <div className="relative">
            {/* Timeline line */}
            <div className={`absolute ${isRTL ? 'right-0 md:right-6' : 'left-0 md:left-6'} top-0 bottom-0 w-px bg-border`} />
            ...
            <div className="space-y-6">
              {experiences.map((exp, i) => (
                ...
                >
                  {/* Timeline dot */}
                  <div className={`absolute ${isRTL ? 'right-0 md:right-6 translate-x-[5px]' : 'left-0 md:left-6 -translate-x-[5px]'} top-2 w-3 h-3 rounded-full bg-primary border-2 border-background`}
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                      <div>
                        <h3 className={`text-lg font-semibold text-foreground flex items-center gap-2 ${isRTL ? 'font-arabic' : 'font-heading'}`}>
                    ...
                    <div className="space-y-2 mb-4">
                      <p className={`text-sm text-muted-foreground ${isRTL ? 'font-arabic' : ''}`}>
                        <span className="font-semibold text-foreground">{t("exp.challenge")}</span> {exp.challenge}
                      </p>
                      <p className={`text-sm text-muted-foreground ${isRTL ? 'font-arabic' : ''}`}>
                        <span className="font-semibold text-primary">{t("exp.solution")}</span> {exp.solution}
                      </p>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {exp.tools.map((tool) => (
                        <span key={tool} className="px-2.5 py-1 text-xs rounded-lg text-primary bg-primary/8 border border-primary/15 font-medium">
                          {tool}
                        </span>
                      ))}
                    </div>
