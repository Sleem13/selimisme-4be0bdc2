...
import { Mail, Phone, Linkedin, PenLine, Sparkles, MessageCircle, Send } from "lucide-react";
    ...
    <section id="contact" className="bg-background">
      <div className="section-padding">
        <div className="max-w-3xl mx-auto text-center">
          ...
          >
            <p className={`text-primary font-heading text-sm tracking-[0.3em] uppercase mb-3 ${isRTL ? 'font-arabic' : ''}`}>{t("contact.label")}</p>
          ...
          >
            ...
            <div className="px-5 py-3 border-b border-border flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-foreground text-sm font-heading font-semibold">{lang === "ar" ? "محمد سليم" : "Mohamed Seliem"}</p>
                <div className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-soft-green" />
                  <span className="text-soft-green text-[10px]">{lang === "ar" ? "متاح للعمل" : "Available for work"}</span>
                </div>
              </div>
            </div>
