...
const EtlPipelineFlow = () => {
  return (
    <div
      className="relative rounded-2xl border border-border bg-card/60 backdrop-blur-sm p-6 md:p-8 overflow-hidden"
      style={{ boxShadow: "var(--shadow-card)" }}
    >
      <div className="flex items-center justify-between mb-5">
        <div>
          <p className="text-[11px] font-heading tracking-[0.3em] uppercase text-primary mb-1">
            Live Data Pipeline
          </p>
        ...
        >
          ...
          {[0, 1, 2, 3, 4].map((i) => (
            ...
            >
              ...
              <animate
                attributeName="fill"
                values="hsl(var(--primary));hsl(var(--gold));hsl(var(--coral));hsl(var(--soft-green))"
                dur="6s"
                repeatCount="indefinite"
                begin={`${i * 1.2}s`}
              />
          ...
          {stages.map((s, i) => (
            ...
            >
              <div
                ...
                style={{
                  background: `${s.color.replace("hsl(", "hsla(").replace(")", ", 0.1)")}`,
                  borderColor: `${s.color.replace("hsl(", "hsla(").replace(")", ", 0.3)")}`,
                }}
              >
                <s.icon className="w-4 h-4" style={{ color: s.color }} />
              </div>
              <p className="text-xs md:text-sm font-heading font-semibold text-foreground leading-tight">
                {s.label}
              </p>
