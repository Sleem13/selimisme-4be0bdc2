...
const AnimatedDataViz = () => (
  <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-[0.07]" viewBox="0 0 800 600">
    {/* Animated line chart paths */}
    <motion.path
      d="M0,400 Q100,350 200,380 T400,300 T600,320 T800,250"
      fill="none"
    ...
    />
    <motion.path
      d="M0,450 Q150,400 300,420 T500,350 T700,370 T800,300"
      fill="none"
    ...
    />
    <motion.path
      d="M0,500 Q200,430 350,460 T550,380 T750,400 T800,340"
      fill="none"
    ...
    {[100, 200, 300, 400, 500, 600, 700].map((x, i) => (
      <motion.rect
        ...
        fill="url(#barGrad)"
        initial={{ height: 0 }}
        animate={{ height: [0, 30 + i * 12, 20 + i * 8] }}
...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      {/* Ambient glow orbs */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(139, 92, 246, 0.12) 0%, transparent 70%)" }} />
      <div className="absolute bottom-1/3 right-1/4 w-80 h-80 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(6, 182, 212, 0.1) 0%, transparent 70%)" }} />
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
        ...
        >
          {/* Gradient circle behind profile */}
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
        >
          <p className={`bg-clip-text text-transparent font-heading text-sm md:text-base tracking-[0.3em] uppercase mb-4 ${isRTL ? 'font-arabic' : ''}`} style={{ backgroundImage: "linear-gradient(135deg, #06b6d4, #8b5cf6)" }}>
            {t("hero.tagline")}
          </p>
        </motion.div>
        ...
        {/* Action headline */}
        <motion.p
          className={`text-xl md:text-2xl font-heading font-semibold mb-6 bg-clip-text text-transparent ${isRTL ? 'font-arabic' : ''}`}
